...
try:
    from app.domain.models import JiraConfig, PendingWorkflowJob, RepoContext, ScmRepoConfig
    ...
    from app.services.workflow_executor import start_workflow_execution
    from app.services.workflow_runtime import WorkflowRuntime
    from app.storage.credential_store import CredentialStore
    from app.storage.workflow_file_store import (
        load_workflow_batch as workflow_store_load_workflow_batch,
        load_workflow_run as workflow_store_load_workflow_run,
        mark_workflow_run_stale as workflow_store_mark_workflow_run_stale,
        save_workflow_batch as workflow_store_save_workflow_batch,
        save_workflow_run as workflow_store_save_workflow_run,
    ...
    if exc.name != "app":
        raise
    from domain.models import JiraConfig, PendingWorkflowJob, RepoContext, ScmRepoConfig
    from routes.config_routes import register_config_routes
    from routes.jira_routes import register_jira_routes
    from routes.page_routes import register_page_routes
    from routes.workflow_routes import register_workflow_routes
    from services.workflow_batch_service import (
        build_batch_preview_items,
        required_batch_workflow_fields,
        resolve_batch_candidates,
    )
    from services.workflow_executor import start_workflow_execution
    from services.workflow_runtime import WorkflowRuntime
    from storage.credential_store import CredentialStore
    from storage.workflow_file_store import (
        load_workflow_batch as workflow_store_load_workflow_batch,
        load_workflow_run as workflow_store_load_workflow_run,
        mark_workflow_run_stale as workflow_store_mark_workflow_run_stale,
        save_workflow_batch as workflow_store_save_workflow_batch,
...
def _workflow_run_snapshot(run: dict[str, Any]) -> dict[str, Any]:
    return workflow_run_snapshot(run, default_agent_provider=DEFAULT_AGENT_PROVIDER)


def _save_workflow_run(run: dict[str, Any]) -> None:
    workflow_store_save_workflow_run(
        run,
        workflow_runs_dir=WORKFLOW_RUNS_DIR,
        lock=WORKFLOW_RUN_FILE_LOCK,
        default_agent_provider=DEFAULT_AGENT_PROVIDER,
    )


def _load_workflow_run(run_id: str) -> dict[str, Any] | None:
    return workflow_store_load_workflow_run(run_id, workflow_runs_dir=WORKFLOW_RUNS_DIR)


def _save_workflow_batch(batch: dict[str, Any]) -> None:
    workflow_store_save_workflow_batch(batch, workflow_batches_dir=WORKFLOW_BATCHES_DIR, lock=WORKFLOW_BATCH_FILE_LOCK)


def _load_workflow_batch(batch_id: str) -> dict[str, Any] | None:
    return workflow_store_load_workflow_batch(batch_id, workflow_batches_dir=WORKFLOW_BATCHES_DIR)


def _workflow_batch_snapshot(batch: dict[str, Any]) -> dict[str, Any]:
    return workflow_batch_snapshot(batch)


def _mark_workflow_run_stale(run: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    current_time = datetime.now(timezone.utc)
    return workflow_store_mark_workflow_run_stale(
        run,
        workflow_stale_seconds=WORKFLOW_STALE_SECONDS,
        now=current_time,
...
_test_changes_plain = _test_changes
...
def create_app() -> Flask:
    ...
    runtime = WorkflowRuntime(
        workflow_runs_dir=WORKFLOW_RUNS_DIR,
        workflow_batches_dir=WORKFLOW_BATCHES_DIR,
        recent_batches_limit=MAX_RECENT_BATCHES,
        stale_statuses={"queued", "running", "needs_input", "pending_plan_review"},
        utcnow_iso=_utcnow_iso,
        new_batch_factory=_new_workflow_batch,
        workflow_run_snapshot=_workflow_run_snapshot,
        workflow_batch_snapshot=_workflow_batch_snapshot,
        workflow_batch_run_ref=_workflow_batch_run_ref,
        workflow_log_ref=_workflow_log_ref,
        workflow_log_sort_timestamp=_workflow_log_sort_timestamp,
        batch_aggregate_status=_batch_aggregate_status,
        batch_suggested_active_run_id=_batch_suggested_active_run_id,
        batch_status_message=_batch_status_message,
        save_workflow_run=_save_workflow_run,
        load_workflow_run=_load_workflow_run,
        save_workflow_batch=_save_workflow_batch,
        load_workflow_batch=_load_workflow_batch,
        mark_workflow_run_stale=_mark_workflow_run_stale,
    )
    workflow_runs = runtime.workflow_runs
    workflow_runs_lock = runtime.workflow_runs_lock
    workflow_batches = runtime.workflow_batches
    workflow_batches_lock = runtime.workflow_batches_lock
    workflow_queue_lock = runtime.workflow_queue_lock
    workflow_queue_pending = runtime.workflow_queue_pending
    workflow_queue_active = runtime.workflow_queue_active
    workflow_run_controls_lock = runtime.workflow_run_controls_lock
    workflow_run_cancel_events = runtime.workflow_run_cancel_events
    workflow_run_processes = runtime.workflow_run_processes

    def _load_batch_file_ids(limit: int | None = None) -> list[str]:
        return runtime.load_batch_file_ids(limit=limit)

    def _load_run_file_ids(limit: int | None = None) -> list[str]:
        return runtime.load_run_file_ids(limit=limit)

    def _iter_known_run_snapshots() -> Iterator[dict[str, Any]]:
        yield from runtime.iter_known_run_snapshots()

    def _recover_batch_from_runs(batch_id: str) -> dict[str, Any] | None:
        return runtime.recover_batch_from_runs(batch_id)

    def _load_active_batch_ids_from_runs(limit: int | None = None) -> list[str]:
        return runtime.load_active_batch_ids_from_runs(limit=limit)

    def _ensure_batch_loaded(batch_id: str) -> dict[str, Any] | None:
        return runtime.ensure_batch_loaded(batch_id)

    def _refresh_batch_state(batch_id: str) -> dict[str, Any] | None:
        return runtime.refresh_batch_state(batch_id)

    def _sync_batch_from_run_snapshot(run_snapshot: dict[str, Any]) -> None:
        runtime.sync_batch_from_run_snapshot(run_snapshot)

    def _register_run(run: dict[str, Any]) -> None:
        runtime.register_run(run)

    def get_run(run_id: str) -> dict[str, Any] | None:
        return runtime.get_run(run_id)

    def update_run(run_id: str, updater: Any) -> dict[str, Any] | None:
        return runtime.update_run(run_id, updater)

    def create_batch(issues: list[dict[str, str]]) -> dict[str, Any]:
        return runtime.create_batch(issues)

    def get_batch(batch_id: str) -> dict[str, Any] | None:
        return runtime.get_batch(batch_id)

    def list_batches(limit: int = MAX_RECENT_BATCHES) -> list[dict[str, Any]]:
        return runtime.list_batches(limit=limit)

    def list_workflow_logs(limit: int = 50) -> list[dict[str, Any]]:
        return runtime.list_workflow_logs(limit=limit)

    def _update_pending_queue_positions(queue_key: str) -> None:
        runtime.update_pending_queue_positions(queue_key, update_run)

    def _ensure_workflow_run_cancel_event(run_id: str) -> threading.Event:
        return runtime.ensure_workflow_run_cancel_event(run_id)

    def _track_workflow_run_process(run_id: str, process: subprocess.Popen[str] | None) -> None:
        runtime.track_workflow_run_process(run_id, process)

    def _clear_workflow_run_controls(run_id: str) -> None:
        runtime.clear_workflow_run_controls(run_id)

    def _request_workflow_run_cancel(run_id: str) -> bool:
        return runtime.request_workflow_run_cancel(run_id, stop_process=_stop_managed_process)

    def _remove_pending_workflow_job(run_id: str, queue_key: str) -> bool:
        return runtime.remove_pending_workflow_job(run_id, queue_key, update_run=update_run)
    ...
    def _start_next_queued_job_locked(queue_key: str) -> PendingWorkflowJob | None:
        return runtime.start_next_queued_job_locked(queue_key, update_run=update_run)

    def enqueue_workflow_run(job: PendingWorkflowJob) -> None:
        runtime.enqueue_workflow_run(
            job,
            normalize_queue_key=_normalize_queue_key,
            update_run=update_run,
            utcnow_iso=_utcnow_iso,
            start_workflow_execution=_start_workflow_execution,
        )

    def _finish_queue_job(queue_key: str) -> None:
        runtime.finish_queue_job(
            queue_key,
            update_run=update_run,
            start_workflow_execution=_start_workflow_execution,
        )
    ...
    def _start_workflow_execution(job: PendingWorkflowJob, queue_key: str) -> None:
        start_workflow_execution(
            job,
            queue_key,
            ensure_cancel_event=_ensure_workflow_run_cancel_event,
            update_run=update_run,
            append_workflow_event=_append_workflow_event,
            set_workflow_status=_set_workflow_status,
            call_with_supported_kwargs=_call_with_supported_kwargs,
            execute_agent_workflow=_execute_agent_workflow,
            track_workflow_run_process=_track_workflow_run_process,
            finish_workflow_run=_finish_workflow_run,
            get_run=get_run,
            safe_record_project_file_map=_safe_record_project_file_map,
            merge_file_map_run_metadata=_merge_file_map_run_metadata,
            safe_record_project_history=_safe_record_project_history,
            clear_workflow_run_controls=_clear_workflow_run_controls,
            finish_queue_job=_finish_queue_job,
            logger=LOGGER,
        )

    def _required_batch_workflow_fields(payload: dict[str, Any]) -> list[str]:
        return required_batch_workflow_fields(payload)
    ...
    def _resolve_batch_candidates(
        ...
        return resolve_batch_candidates(
            issues,
            common_payload,
            jira_payload,
            scm_payload,
            load_repo_context=_load_repo_context,
            repo_context_requested_fields=_repo_context_requested_fields,
            build_requested_information=_build_requested_information,
            safe_ensure_project_memory=_safe_ensure_project_memory,
            safe_fetch_issue_detail=_safe_fetch_issue_detail,
            batch_issue_workflow_payload=_batch_issue_workflow_payload,
            normalize_queue_key=_normalize_queue_key,
            resolve_commit_identity=_resolve_commit_identity,
        )
    ...
    def _build_batch_preview_items(issues: list[dict[str, str]], scm_payload: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
        return build_batch_preview_items(
            issues,
            scm_payload,
            load_repo_context=_load_repo_context,
            repo_context_requested_fields=_repo_context_requested_fields,
            build_requested_information=_build_requested_information,
            normalize_queue_key=_normalize_queue_key,
            batch_tab_label=_batch_tab_label,
            suggest_branch_name=_suggest_branch_name,
        )

    register_page_routes(
        app,
        setup_guide_payload=SETUP_GUIDE,
        agentation_frontend_config=_agentation_frontend_config,
        load_codex_cli_defaults=_load_codex_cli_defaults,
        load_claude_cli_defaults=_load_claude_cli_defaults,
        valid_reasoning_efforts=VALID_REASONING_EFFORTS,
        valid_claude_permission_modes=VALID_CLAUDE_PERMISSION_MODES,
        default_agent_provider=DEFAULT_AGENT_PROVIDER,
        agent_provider_options_payload=_agent_provider_options_payload,
    )

    register_config_routes(
        app,
        store=store,
        scm_store_key=SCM_STORE_KEY,
        load_scm_payload=_load_scm_payload,
        normalize_repo_mapping_token_map=_normalize_repo_mapping_token_map,
        normalize_jira_jql_mode=_normalize_jira_jql_mode,
        normalize_jira_jql_builder=_normalize_jira_jql_builder,
        open_directory_picker=_open_directory_picker,
        required_config_fields=_required_config_fields,
        repo_mapping_missing_token_spaces=_repo_mapping_missing_token_spaces,
        build_requested_information=_build_requested_information,
        parse_repo_mappings=_parse_repo_mappings,
        normalize_space_key_list=_normalize_space_key_list,
        effective_secret_value=_effective_secret_value,
        to_jira_config=_to_jira_config,
        normalize_base_url=_normalize_base_url,
        serialize_repo_mappings=_serialize_repo_mappings,
    )

    register_jira_routes(
        app,
        store=store,
        fetch_jira_projects=_fetch_jira_projects,
        fetch_jira_users=_fetch_jira_users,
        fetch_all_jira_backlog_issues=_fetch_all_jira_backlog_issues,
        fetch_jira_issue_detail=_fetch_jira_issue_detail,
        build_requested_information=_build_requested_information,
        effective_secret_value=_effective_secret_value,
        jira_status_filter_options=list(JIRA_STATUS_FILTER_OPTIONS),
        jira_sort_direction_options=list(JIRA_SORT_DIRECTION_OPTIONS),
        mock_jira_issue_detail=_mock_jira_issue_detail,
    )
    ...
    register_workflow_routes(
        ...
        load_codex_cli_defaults=_load_codex_cli_defaults,
        load_claude_cli_defaults=_load_claude_cli_defaults,
        provider_option_payload=_provider_option_payload,
        agent_provider_options_payload=_agent_provider_options_payload,
        load_repo_context=_load_repo_context,
        repo_context_requested_fields=_repo_context_requested_fields,
        build_requested_information=_build_requested_information,
        safe_ensure_project_memory=_safe_ensure_project_memory,
        scm_repo_status=_scm_repo_status,
        repo_dirty_entries=_repo_dirty_entries,
        resolve_commit_identity=_resolve_commit_identity,
        git_optional_output=_git_optional_output,
        required_batch_workflow_fields=_required_batch_workflow_fields,
        normalize_batch_issues=_normalize_batch_issues,
        build_batch_preview_items=_build_batch_preview_items,
        suggest_branch_name=_suggest_branch_name,
        normalize_workflow_agent_payload=_normalize_workflow_agent_payload,
        normalize_clarification_requests=_normalize_clarification_requests,
        normalize_clarification_answers=_normalize_clarification_answers,
        required_workflow_fields=_required_workflow_fields,
        payload_with_hydrated_clarification_context=_payload_with_hydrated_clarification_context,
        merge_clarification_questions=_merge_clarification_questions,
        merge_clarification_answers=_merge_clarification_answers,
        agent_execution_validation_error=_agent_execution_validation_error,
        agent_cli_missing_error=_agent_cli_missing_error,
        clarification_error_payload=_clarification_error_payload,
        provider_launcher=_provider_launcher,
        run_agent_clarification=_run_agent_clarification,
        run_agent_plan_review=_run_agent_plan_review,
        plan_review_payload=_plan_review_payload,
        set_run_pending_plan_review=_set_run_pending_plan_review,
        safe_sync_jira_clarification_questions=_safe_sync_jira_clarification_questions,
        safe_sync_jira_clarification_answers=_safe_sync_jira_clarification_answers,
        resolve_batch_candidates=_resolve_batch_candidates,
        create_batch=create_batch,
        new_workflow_run=_new_workflow_run,
        append_workflow_event=_append_workflow_event,
        ...
        cancel_workflow_run_now=_cancel_workflow_run_now,
        remove_pending_workflow_job=_remove_pending_workflow_job,
        request_workflow_run_cancel=_request_workflow_run_cancel,
        normalize_queue_key=_normalize_queue_key,
        utcnow_iso=_utcnow_iso,
        finish_workflow_run=_finish_workflow_run,
        clarification_execution_error=ClarificationExecutionError,
        enqueue_workflow_run=enqueue_workflow_run,
        pending_workflow_job_factory=PendingWorkflowJob,
        store=store,
    )
