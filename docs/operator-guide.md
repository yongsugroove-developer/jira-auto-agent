...
- Jira API Token은 저장 후 입력칸이 비어 보여도 기존 값이 유지될 수 있다.
- 빈 값으로 다시 저장해도 기존 저장 토큰을 자동으로 제거하지 않는다.

### 4.2 공간별 저장소 연결

현재 버전은 전역 기본 저장소를 쓰지 않는다.  
모든 실행은 Jira 공간 키별 저장소 연결이 있어야 한다.
...
- 이슈 키의 공간 접두어와 저장소 연결의 공간 키가 정확히 일치해야 한다.
...
`백로그 조회`는 저장된 JQL 기준으로 전체 이슈를 가져온다.

- 목록은 고정 높이 스크롤 영역이다.
- 조회 직후 기본 선택된 이슈는 없다.
- 선택 이슈 상세와 최근 코멘트는 리스트 내부 아코디언으로만 보여준다.

### 5.2 이슈 선택

- 체크박스로 여러 이슈를 선택할 수 있다.
- 선택 요약 영역에서 건수와 이슈 키를 확인할 수 있다.
- `선택 해제`로 초기화할 수 있다.
...
- 선택한 이슈 공간 키에 대한 저장소 연결이 있는지 확인한다.
- 로컬 저장소가 깨끗한지 확인한다.
- 선택한 provider 실행기가 준비되어 있는지 확인한다.

### 6.4 hidden test_command

- `test_command`는 현재 화면에 직접 노출하지 않는다.
- 기존 저장값이 있으면 payload 호환성과 결과 요약에는 남는다.
- 현재 기본 검증은 주로 변경 파일 문법 검사 중심이다.
...
clarification이 발생하면 질문 탭에서 답변 후 재개한다.
...
프로젝트 메모리는 저장소 공통이 아니라 `저장소 경로 + Jira 공간 키` 기준으로 분리한다.

의미:

- 같은 저장소라도 공간 키가 다르면 메모리 버킷이 분리된다.
- 작업 완료 이력은 공간별 project memory에 누적된다.
- 다음 LLM 작업 프롬프트는 해당 공간의 최신 메모리를 우선 사용한다.
...
- 작업 대상 저장소에 tracked, staged, untracked 변경이 남아 있음

대응:

- 런타임 산출물이 아닌 변경은 먼저 정리한다.
- 런타임 JSON이라면 `.gitignore`와 dirty 체크 예외 범위에 있는지 확인한다.

### 10.3 Claude 실행기 준비 실패
